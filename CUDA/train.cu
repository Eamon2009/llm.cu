...
void gpt_build_from_descriptor(GPT2 *model, const char *descriptor)
{
      ...
      for (int l = 0; l < L; l++)
      {
            ...
            for (int i = 0; i < NUM_PARAMETER_TENSORS; i++)
            {
                  ...
                  if (l == 0 && (i == 2 || i == 8 || i == 14))
                  { // only at l = 0 to init these just once
...
int main(int argc, char *argv[])
{
      ...
      for (int i = 1; i < argc; i += 2)
      {
            ...
            {
                  error_usage();
            } // must be -x[y] (one dash, one or two letters)
            // read in the args
            if (argv[i][1] == 'i')
            {
      ...
      {
            gelu_fusion = 0;
      } // (deviceProp.major >= 9) ? 2 : 0; } // in gpt2_init_common for test_gpt2cu...
      ...
      for (; step <= train_num_batches; step++)
      {
            ...
            if (step > 0)
            { // consider the first batch to be a warmup (e.g. cuBLAS/cuDNN initialisation)
